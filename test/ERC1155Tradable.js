...
contract("ERC1155Tradable - ERC 1155", (accounts) => {
	...
	describe("#create()", () => {
		...
		it("should allow the contract owner to create tokens and emit a URI", async () => {
			tokenId += 1;
			truffleAssert.eventEmitted(
				await instance.create(owner, 0, vals.URI_BASE, "0x0", { from: owner }),
				"URI",
				{
					_uri: vals.URI_BASE,
					_id: toBN(tokenId)
	...
	describe("#setCreator()", () => {
		...
		it("should not allow the token creator to set creator to 0x0", () =>
			truffleAssert.fails(
				instance.setCreator(vals.ADDRESS_ZERO, [INITIAL_TOKEN_ID], {
					from: creator
				}),
				truffleAssert.ErrorType.revert,
		...
		it("should not allow a non-token-creator to set creator", // Check both a user and the owner of the contract
		async () => {
	...
	describe("#mint()", () => {
		it("should allow creator to mint tokens", async () => {
			await instance.mint(userA, INITIAL_TOKEN_ID, MINT_AMOUNT, "0x0", {
				from: creator
			});
		...
		it("should not overflow token balances", async () => {
			...
			await truffleAssert.fails(
				instance.mint(userB, INITIAL_TOKEN_ID, OVERFLOW_NUMBER, "0x0", {
					from: creator
				}),
				truffleAssert.ErrorType.revert,
				"OVERFLOW"
	...
	describe("#batchMint()", () => {
		...
		it("should require that caller has permission to mint each token", async () =>
			truffleAssert.fails(
				instance.batchMint(userA, [INITIAL_TOKEN_ID], [MINT_AMOUNT], "0x0", {
					from: userB
				}),
				truffleAssert.ErrorType.revert,
